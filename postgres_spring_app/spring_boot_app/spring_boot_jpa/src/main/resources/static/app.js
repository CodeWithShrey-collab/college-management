...
async function initAssistantPage() {
  ...
  bindForm('chat-form', async (payload, form) => {
    const result = await callAssistant(payload.question);
...
function resetAssistant() {
  ...
  chatOutput.innerHTML = `
    ...
    <p>Try prompts around exams, attendance, fee status, notices, or library circulation. The assistant is powered by the server-side NVIDIA integration.</p>
  `;
}
  ...
  document.querySelectorAll('.prompt-chip').forEach((button) => {
    button.addEventListener('click', async () => {
      await runAction(async () => {
        const result = await callAssistant(button.dataset.prompt);
        renderChat(result.question, result.answer, result.reasoning);
      });
    });
  });
...
async function callAssistant(question) {
  return postJson('/api/erp/chat', { question });
}

function renderAssistantConnectionStatus() {
  const node = document.getElementById('assistant-connection-status');
  if (!node) return;
  node.innerHTML = 'Model provider: <strong>NVIDIA API</strong><br>Assistant requests are proxied through your Spring backend.';
}
...
async function postJson(url, body) {
  ...
  if (!response.ok) {
    throw new Error(data.error || data.message || 'Unable to complete the request.');
  }
